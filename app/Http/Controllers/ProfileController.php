...
class ProfileController extends Controller
{
    ...
    {
        $data = [];
        $data['listings'] = $user->listings()->with('user')->with('pricing_model')->orderBy('created_at', 'DESC')->active()->get();
        $data['profile'] = $user;
        $data['rate'] = $request->get('rate', '');
        $data['filters'] = Filter::orderBy('position', 'ASC')->where('is_hidden', 0)->where('is_default', 0)->get();;
      
        $comments = BitDeals::feedback_list(array('account' => $user->bitcoin, 'status' => (int)$data['rate']));
      
        $data['bd_comments'] = $comments;
      
        if(!is_array($comments)) {
        if ((int)$data['rate']) {
            $data['comments'] = $user->comments()->with('commenter')->with('listing')->where('rate', $data['rate'])->paginate(20);
        } else {
            $data['comments'] = $user->comments()->with('commenter')->with('listing')->paginate(20);
        }
        } else {
            foreach ($comments as &$comment) {
                $comment->date = date('Y-m-d', strtotime($comment->date));
            }
            $data['bd_comments'] = array_reverse($comments);
        }
        ...
        MetaTag::set('rss', url('/feed') . '?user=' . $user->id);
