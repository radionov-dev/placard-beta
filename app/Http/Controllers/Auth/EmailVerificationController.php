...
class EmailVerificationController extends Controller
{
    ...
    public function index() {
        ...
        MetaTag::set('title', __("Verification"));
        if(!session()->has('from')){
    ...
    public function resend(Request $request) {
        ...
        if ($can_resend) {
            ...
            UserVerification::generate(auth()->user());
            $user = auth()->user();
            if (!empty($user->bitmessage)) {
                Bit_Message::send($user->bitmessage, __('Welcome and Verification'), __("Hello $user->username, you have been registered at " . config('app.name') . ".\n\rPlease confirm you e-mail address by follow the link: " . route('email-verification.check', $user->verification_token) . '?email=' . urlencode($user->email)));
            } else {
                UserVerification::send(auth()->user(), __(':site_name verification code', ['site_name' => setting('site_name')]));
            }

            alert()->success(__('Verification was resent.'));
        } else {
            alert()->danger(__('Please wait up to 5 minutes before requesting a new verification.'));
        }
