...
class User extends Authenticatable implements BannableContract, JWTSubject, PlanSubscriberInterface
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'password', 'avatar', 'bio', 'display_name', 'gender', 'phone', 'city', 'region', 'country', 'unread_messages', 'username', 'provider', 'provider_id', 'bitcoin', 'bitmessage', 'currency'
    ];
     ...
     */
    protected $hidden = [
        'password', 'remember_token', 'facebook_app_key', 'facebook_app_secret', 'provider', 'provider_id', 'email', 'location', 'bitcoin', 'bitmessage'
    ];
    protected $append = [
        'first_name'
    ];
    ...
    public function getAvatarAttribute() {
        if(!$this->attributes['avatar']) {
            ...
            return "https://ui-avatars.com/api/?size=256&length=5&format=svg&font-size=0.20&bold=true&background=".$background."&color=fff&name=".urlencode($this->bitcoin);
    ...
    public function messagies()
    {
        return $this->hasMany(Message::class, 'user_id');
    }

    public function getUnreadMessagesCount() {
        return $this->messagies()->getUnreadMessagesCount();
    }
    /*
