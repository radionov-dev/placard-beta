...
class Listing extends Model implements FeedItem
{
    #use Eloquence;
    use MergedTrait;
    use SearchableTrait;
    ...
    protected $fillable = [
        'key', 'title', 'subtitle', 'price', 'stock', 'unit', 'category_id', 'user_id', 'short_address', 'description', 'spotlight', 'staff_pick', 'is_hidden', 'location', 'lat', 'lng', 'pricing_model_id', 'photos', 'city', 'region',  'country', 'currency', 'is_draft', 'session_duration', 'min_duration', 'max_duration'
    ];
    protected $casts = [
          'photos' => 'array',
          'meta' => 'json',
    ...
    public function getPriceFormattedAttribute() {
        ...
        if ($this->price_in_btc == 1) {
            $convert = new Converter;

            $this->price = $convert->toCurrency($this->currency, (float)$this->btc_price);
            $this->save();
        }

        if($this->price) {
            $price = format_to_money($this->price, $this->currency, current_currency());
		}
    ...
    public function getPriceUpdatedAttribute() {
        $price = $this->price;

        if ($this->price_in_btc == 1) {
            $convert = new Converter;
            $price = $convert->toCurrency($this->currency, (float)$this->btc_price);
            $this->price = $price;
            $this->save();
        }
    ...
    {
        $request = request();
        $listing_data = new BrowseController();
        return $listing_data->getListingData($request, 1);
